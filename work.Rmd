...
these additional variables and only keep ones that have values for every
observation.

Now, all variables of each window are identified  as the same class (calculation
...
This is even more clearly shown as a chart.

```{r}
training %>% with(qplot(X, num_window, color=classe, main="text")) %>% print
```

We also remove non-sensor measurements: line numbers, user names,
timestamps and new window markers.
```{r, cache=TRUE}
